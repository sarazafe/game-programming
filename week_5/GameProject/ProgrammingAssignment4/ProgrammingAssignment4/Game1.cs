        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            else if (mouse.RightButton == ButtonState.Released)
            {
                ...
                if (rightClickStarted)
                {
                    ...
                    Pickup pickup = new Pickup(pickupSprite, location);
                    pickups.Insert(pickups.Count, pickup);
                ...
                teddy.CollisionRectangle.Intersects(pickups[0].CollisionRectangle))
            {
                ...
                if(pickups.Count > 0)
                {
                    Vector2 pickupVector = new Vector2(pickups[0].CollisionRectangle.X, pickups[0].CollisionRectangle.Y);
                    teddy.SetTarget(pickupVector);
                }else
                {
                    teddy.ClearTarget();
