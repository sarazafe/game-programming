    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                ...
                {
                    TeddyBear oneTeddy = bears[i];
                    TeddyBear theOtherTeddy = bears[j];
                    if (oneTeddy.Active && theOtherTeddy.Active)
                    {
                        CollisionResolutionInfo collisionResolutionInfo = CollisionUtils.CheckCollision(gameTime.ElapsedGameTime.Milliseconds, GameConstants.WindowWidth, GameConstants.WindowHeight, oneTeddy.Velocity, oneTeddy.DrawRectangle, theOtherTeddy.Velocity, theOtherTeddy.DrawRectangle);
                        if (null != collisionResolutionInfo)
                        {
                            // first teddy collision
                            if (collisionResolutionInfo.FirstOutOfBounds)
                            {
                                oneTeddy.Active = false;
                            }
                            else
                            {
                                oneTeddy.Velocity = collisionResolutionInfo.FirstVelocity;
                                oneTeddy.DrawRectangle = collisionResolutionInfo.FirstDrawRectangle;

                                // adding sound effect
                                teddyBounce.Play();
                            }

                            // second teddy collision
                            if (collisionResolutionInfo.SecondOutOfBounds)
                            {
                                theOtherTeddy.Active = false;
                            }
                            else
                            {
                                theOtherTeddy.Velocity = collisionResolutionInfo.SecondVelocity;
                                theOtherTeddy.DrawRectangle = collisionResolutionInfo.SecondDrawRectangle;
            ...
            int initialBurguerHealth = burger.Health;
            foreach (TeddyBear bear in bears){
            ...
            foreach(Projectile projectile in projectiles)
            {
                if (projectile.Active && ProjectileType.TeddyBear.Equals(projectile.Type) && projectile.CollisionRectangle.Intersects(burger.CollisionRectangle))
                {
            ...
            // if health has changed, update health string
            if(initialBurguerHealth != burger.Health)
            {
                UpdateHealthString();
            }
            ...
            foreach (TeddyBear bear in bears)
            {
                foreach (Projectile projectile in projectiles)
                {
                    // if projectile is french frye and projectile collisions with teddy, deactivate both of them
                    if (ProjectileType.FrenchFries.Equals(projectile.Type) && bear.CollisionRectangle.Intersects(projectile.CollisionRectangle))
                    {
                        // inactive teddy and projectile
                        projectile.Active = false;
                        bear.Active = false;
                        explosions.Add(new Explosion(explosionSpriteStrip, bear.CollisionRectangle.Center.X, bear.CollisionRectangle.Center.Y, explosion));
        ...
        public static Texture2D GetProjectileSprite(ProjectileType type)
        {
            ...
            else if (ProjectileType.TeddyBear == type)
            {
                return teddyBearProjectileSprite;
            }
            else
            {// by default throw french fries
                return frenchFriesSprite;
            }
        }
        ...
        private void SpawnBear()
        {
            ...
            float angle = (float)(2 * RandomNumberGenerator.NextFloat((float)Math.PI) * Math.PI);
        ...
        /// <summary>
        /// Updates the string with the current health of the burguer
        /// </summary>
        ...
        /// <summary>
        /// Updates the string with the current score
        /// </summary>
